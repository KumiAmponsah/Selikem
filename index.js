...
async function main() {
  ...
  const ffmpegPath = require('ffmpeg-static'); // This will point to the static FFmpeg binary
    ...
    try {
      ...
      if (searchResponse.data.tracks.items.length > 0) {
        ...
        return {
          ...
          lyrics: 'No lyrics available from Spotify.'
        };
      }
    } catch (error) {
    ...
    try {
      ...
      if (searchResponse.data.results.length > 0) {
        ...
        return {
          ...
          lyrics: 'No lyrics available from Apple Music.'
        };
      }
    } catch (error) {
  ...
  const getExternalMetadata = async (acrId, artist, title) => {
    ...
    let { song_url, album_art_url, lyrics } = externalMetadata;
    ...
    return { song_url, album_art_url,  lyrics };
  };
